...
> MacBook은 덮개를 닫아도 잠자기 상태에서 네트워크가 유지되므로 접속 가능합니다 (다만 배터리 소모 주의).

### Mac mini: 시간 기반 잠자기 스케줄

항상 켜두는 대신, 특정 시간대에만 깨어있도록 스케줄을 설정할 수 있습니다:

```bash
# 매일 오전 10시 기상, 오후 10시 잠자기
...
> 스케줄 외 시간에는 Mac mini가 잠자기 상태로 접속 불가. 접속 시간대를 감안해서 사용할 것.
